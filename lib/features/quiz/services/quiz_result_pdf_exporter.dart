...
class QuizResultPdfExporter {
  ...
  Future<File> export({
    ...
    // Building the PDF (looping over every question, laying out text,
    // encoding the document) is synchronous CPU-bound work — on a long
    // quiz with verbose AI feedback this can run for long enough to block
    // the UI isolate. Running it in a background isolate via compute()
    // keeps the app responsive regardless of quiz size.
    final bytes = await compute(_buildPdfBytes, _PdfExportArgs(quiz, answers));
...
class _PdfExportArgs {
  const _PdfExportArgs(this.quiz, this.answers);

  final QuizResponse quiz;
  final Map<int, QuizAnswer> answers;
}

Future<Uint8List> _buildPdfBytes(_PdfExportArgs args) async {
